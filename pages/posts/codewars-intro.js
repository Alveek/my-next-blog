...
export default function CodewarsIntro() {
	...
	return (
		<Box as="article">
			...
			<Heading as="h1" mb={10} size="2xl" textAlign="center">
				Codewars: обзор и рекомендации
			</Heading>
			<Text>
				Когда вы выбрали для себя основной язык программирования, то необходимо
				закрепить его основы: синтаксис, встроенные методы, принципы
				программирования в целом. Для этого лучше всего, на мой взгляд подходит
				решение задач.
			</Text>
			<Text>
				Конечно, это может звучать не так весело, но на практике - очень
				действенный способ. Кто-то может подумать, что “задачи - это не моё”. Но
				программирование - это и есть решение одной проблемы за другой с помощью
				написания алгоритмов, используя синтаксис конкретного языка. Если вы не
				захотите изменить свое мнение относительно решения задач, возможно вам
			...
			<Text mt={7}>
				...
				<Code colorScheme={codeColor}>multiply</Code>, суть которой умножить два
				числа друг на друга. В <b>JavaScript</b>, чтобы функция возвращала
	...
	return a * b;
			...
			>
				<Text mb={0}>
					...
					войти под учетной записи GitHub.)
				</Text>
			</Box>
			<Text mt={7}>
			...
			<Text>
				Итак, войдя под своей учетной записью вы попадаете на страницу, где вам
				будет предложена следующая задача для решения. Но лучше перейти к списку
				всех задач и выбрать для себя подходящие. В панели слева нажмите на
				Kata. Слово “ката” взято из восточных боевых искусств и в данном случае
				употребляется как{" "}
				<em>
			...
			<Text>
				...
				уровней. 8 kyu - самый простой и 1 kyu самый сложный. Термин <b>kyu</b>{" "}
				(кю) - также взят из карате и означает сколько степеней еще осталось до
				мастера (1 dan).
			</Text>
			<Text>
			...
			<UnorderedList pl="5" mb="10">
				...
				<ListItem>
					<b>Progress:</b> Kata I have not trained on (Каты, которые я не решал)
				</ListItem>
				<ListItem>
			...
			<Text>
				Добавьте эту страницу в закладки. Так не придется каждый раз выставлять
				эти настройки и всегда будет предложена актуальная для вас задача.
			</Text>
			<Text>
				Решать задачи, с которыми справились большинство участников - это почти
				гарантия, что в ней отсутствуют недочеты, а также то, что вы её скорее
				всего решите сами, и как следствие - захотите ещё.
			</Text>
			<Text>
			...
			<OrderedList pl="5" mb="10">
				...
				<ListItem>
					Во вкладке <b>Solutions</b> можно подсмотреть решения, но тогда вам не
					засчитают очки.
				</ListItem>
				<ListItem>
			...
			<OrderedList pl="5" mb="10">
				...
				<ListItem>
					...
					всегда нажимаю сразу на эту кнопку, потому что я могу пройти тесты из
					пункта 3, но не пройти случайные тесты, так что смысла в{" "}
					<b>RUN SAMPLE TESTS</b> практически нет.
				</ListItem>
			</OrderedList>

			<Text>
				Писать код можно прямо на месте (в окне, помеченным номером 2) и тут же
			...
			<OrderedList pl="5" mb="10">
				...
				<ListItem>
					Показывается количество пройденных тестов. Когда все тесты подсвечены
					зеленым цветом, это значит, что они пройдены. Если красным, то не
					пройдены.
				</ListItem>
				<ListItem>
			...
			<Text>
				...
				решениями остальных участников. Первыми идут самые популярные, с высоким
				рейтингом. Сразу хочу предупредить, что чаще всего в первых рядах будут
				решения длинною в несколько строк. Это больше выглядит на шифр, который
				иногда трудно разобрать. Пусть такие решения не наводят вас на мысль,
				что вы, наверное, плохой программист. Поначалу вы можете просто не знать
				какие-то встроенные методы или регулярные выражения, которые сильно
				сокращают код. Попробуйте разобраться хотя бы с одним таким “умным”
				кодом и решить задачу еще раз, но уже с новыми знаниями.
			</Text>
			...
			<OrderedList pl="5" mb="10">
				...
				<ListItem>
					...
					объективно. Например, 419 человек отметили, что решение придерживается
					лучших практик написания кода. Но в нем отсутствует, например,
					проверка на тип введенных данных пользователем. Если он введет вместо
					числа какое-нибудь слово, то код уже не будет работать. Напомню, что в
					данном случае речь идет о <b>JavaScript</b>.
				</ListItem>
				<ListItem>
			...
			<OrderedList pl="5" mb="10">
				...
				<ListItem>Уведомления, которые будут появляться крайне редко.</ListItem>
				<ListItem>
					Текущий разряд. 8 кю самый низкий, 1 кю - самый высокий, а после 1 кю
					идут 1 - 4 дан, это уже мастера.
				</ListItem>
				<ListItem>
					...
					привилегий, которыми вы пользоваться скорее всего не будете. Например,
					скрывать самостоятельно комментарии, если они содержат спойлер.
				</ListItem>
			</OrderedList>
			...
			<OrderedList pl="5" mb="10">
				...
				<ListItem>
					Визуальное представление сколько еще осталось до следующего разряда. В
					моем случае я прошел только 15,2%.
				</ListItem>
			</OrderedList>

			<Text>
				Несколько слов о рейтинге. Несмотря на то, что на данный момент у меня 4
				kyu, фактически я не могу еще решать задачи с таким же уровнем
				...
				конкретных задач. Например, если я решил 30 задач 6 kyu, мне присваивают
				6 разряд. А так, решая задачи только для 8 разряда вы получите 6-й.
				Просто это будет дольше. Чем ниже сложность задачи, тем меньше очков она
				дает.
			</Text>
			...
			<Text>
				...
				элегантным и коротким, но зато вы нашли нужный алгоритм самостоятельно и
				повысили свой уровень решения задач. А это самое главное. Сделать
				рефакторинг кода никогда не будет поздно. Игнорируйте мысли о том ,что
				всё должно быть идеально с самого начала. Особенно интересно получается,
				когда вы видите другое хорошее решение, в этот момент новая информация
				закрепляется намного эффективнее, чем, если бы узнали её не промучившись
				целый день над своим решением.
			</Text>

			<Text>
				Также, берите задачи, которые вам больше нравятся. К примеру, я
				пропускаю математические задачи, потому что даже не понимаю о чем в них
				речь вообще. Это касается не только математических задач - иногда
				описание бывает очень плохим. В этом случае может быть полезным почитать
				дискуссию, где, возможно кто-то оставил более понятное объяснение.
			</Text>
